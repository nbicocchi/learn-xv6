...
trap(struct trapframe *tf)
{
  if(tf->trapno == T_SYSCALL){
   ...
   /* 
    * Lazy page allocation to respond to a page fault from user space by mapping a newly-allocated page 
    * of physical memory at the faulting address, and then returning back to user space to let the process
    * continue executing 
    */
