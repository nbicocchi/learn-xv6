...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    ...
    if (ip->type == T_CHECKED) {
      ...
      {
        if((ip->addrs[i] & 0x00ffffff)== block){			// considero solo i 3 byte del puntatore che coincidano, al contrario di prima
          isFind = 1;
          addr = ip->addrs[i];								// assegno l'indirizzo
      ...
      if (isFind == 0 && ip->addrs[NDIRECT]!=0 ) {
        ...
        {
          if(((*ptr) & 0x00ffffff) == block){				// stessa condizione 
            addr = *ptr;									// assegno l'indirizzo
          }
        }
        brelse(buf);       
      } 
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
	...
	if (ip->type == T_CHECKED) {
      ...
      {
        if((ip->addrs[i] & 0x00ffffff) == block){			// stessa verifica delle readi
          blockNum = i;
          break;
        }  
      }
      if (blockNum > -1) {
        ...
        {
            if(((*ptr) & 0x00ffffff) == block){				// come sopra
            *ptr = block + (checksum << 24);				// bit dell'indirizzo del blocco più quelli del checksum  right shift by 24 
															// per lasciare spazio ai 3 byte ricordando la spiegazione di prima
            break;
          }
          ptr++;
        }
        bwrite(indirect_buf);								// scrivo un blocco e attendo che l'I/O abbia finito, a meno che 
															// la flag B_ASYNC non sia settata
        brelse(indirect_buf);
      }      
    }
    //Fine modifica
