...
void* shmgetat(int key, int num_pages){
  ...
  if (num_pages < 1 || num_pages > NKEYPAGES) {
    return 0;
  }
  // Allocate memory if key hasnt been used yet
  if (key_refcount[key] == 0) {
    ...
    //Pages allocated for the key
    key_numpages[key] = num_pages;

    //key now used by the process
    key_refcount[key]++;
    proc->keys[key] = 1;
  } else { 
	 //Check if process already called shmgetat
   	 if (proc->keys[key] == 0) { 
   	   for(i = 0; i < num_pages; i++) {
    	    proc->top -= PGSIZE;
    	    addr = (void*)(proc->top);
...
dec_ref_count(struct proc *p)
{
  ...
  for(i = 0; i < NKEYS; i++) {
    if(p->keys[i] == 1 && key_refcount[i]!=0) {
      key_refcount[i]--;
      if(key_refcount[i] == 0) {
...
int shm_refcount(int key){
	return key_refcount[key];
}

void shmeminit(){
	cprintf("Initializing structures for shared memory\n");
	cprintf("Nkeys: %d\n",NKEYS);
	cprintf("Nkeypage: %d\n",NKEYPAGES);
