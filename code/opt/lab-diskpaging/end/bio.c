...
bget(uint dev, uint blockno)
{
  ...
  // Not cached; recycle an unused buffer.
  // A random number is generated and the corresponding block is replaced.

  int vittima = random(NBUF);
  
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
     if(b == &bcache.buf[vittima]){
      b->dev = dev;
      b->blockno = blockno;
      b->flags = 0;
      b->refcnt = 1;
...
bget(uint dev, uint blockno)
{
  ...
  // Not cached; recycle an unused buffer.
  // The block that has long been in memory is replaced

  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
     if(b == &bcache.buf[pos-1] ){
